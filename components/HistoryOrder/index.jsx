...
const HistoryOrder = ({ accessToken }) => {
  ...
  useEffect(() => {
    const detailOrder = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:5000/v1/api/get-order/${selectedOrderId ? selectedOrderId : dataId}`, {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
        });
              ...
              <Col md={5}>
                <Card>
                  <div className="mt-4 p-2">
                    ...
                    <div>
                      <div className="d-flex">
                        {order.tiket && <p className="bg-transparent fw-bold border border-light">{order.tiket.jam_berangkat}:00</p>}
                        <p className={`${styles.spanHarga} bg-transparent fw-bold ms-auto border border-light`}>Kedatangan</p>
                      </div>
                      {order.tiket && <div className={`ms-3`}>{order.tiket.tanggal_berangkat}</div>}
                      {order.tiket && (
                        <div>
                          <div className={`ms-3`}>{order.tiket.bandaraAwal.nama_bandara}</div> <hr />
                        </div>
                      )}
                      {order.maskapai && (
                        <div>
                          <div className={`fw-bold ms-5`}>
                            {order.maskapai.nama_maskapai} - {order.maskapai.tipe_maskapai}
                          </div>
                          <div className={`fw-bold ms-5`}>{order.maskapai.kode_maskapai}</div>
                        </div>
                      )}
                      ...
                      <hr />
                      <div className="d-flex">
                        {order.tiket && <p className="bg-transparent fw-bold border border-light">{order.tiket.jam_kedatangan}:00</p>}
                        <p className={`${styles.spanHarga} bg-transparent fw-bold ms-auto border border-light`}>Kedatangan</p>
                      </div>
                      {order.tiket && <div className={`ms-3`}>{order.tiket.tanggal_kedatangan}</div>}
                      {order.tiket && (
                        <div>
                          <div className={`ms-3`}>{order.tiket.bandaraTujuan.nama_bandara}</div> <hr />
                        </div>
                      )}
                      <div>
                        ...
                        <div className="d-flex">
                          ...
                          {order.tiket && (
                            <div className="ms-auto">
                              <p className={`bg-transparent fw-bold ms-auto border border-light  text-end`}>{order.tiket.maskapai.harga_tiket}</p>
                            </div>
                          )}
                        </div>
                        <div className="d-flex">
